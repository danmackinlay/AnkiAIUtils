...
class AnkiIllustrator:
    ...
    def __init__(
        ...
        dataset_path: str
            ...
            prompt) showing the reasoning to create illustrations
        ...
        for i, note in tqdm(
            ...
            try:
                ...
                input_token_cost, output_token_cost, image_prompt, reasoning, discarded = (
    ...
    def _update_anki_note(
        ...
        for i, d in enumerate(imgs_dict):
            ...
            if self.debug:
                ...
                field_content += f'REASONING: {escape(reasoning)}\n\n'
        ...
        reason_lines = escape(reasoning).splitlines()
        filtered_reason = []
    ...
    def _create_prompt(self, note_info: Dict):
        ...
        reasoning, image_prompt, discarded_text = parse_llm_answer(response)
        whi(f"\n* Note formatted content: '{note_info['formatted_content']}'")
        whi(f"\n* Reasoning: '{reasoning}'")
        whi(f"\n* Prompt: '{image_prompt}'")
        whi(f"\n* Token cost: {input_token_cost} + {output_token_cost}")
        return input_token_cost, output_token_cost, image_prompt, reasoning, discarded_text
...
def parse_llm_answer(response : Dict) -> Tuple[str, str, str]:
    ...
    try:
        ...
        reasoning, prompt = sp[0].strip(), sp[1].strip()
        assert reasoning.strip(), f"Invalid LLM answer: empty reasoning: {reasoning}\nFull answer:\n'{prp}'"
        ...
        for li in lines:
            if li.startswith("Topic:"):
                reasoning.append(li)
                assert not discarded_after, f"Invalid LLM answer: found lines to discard before reasoning:\nFull answer:\n'{prp}'"
            elif not reasoning:
                ...
                assert reasoning, f"Invalid LLM answer: found image prompt before reasoning:\nFull answer:\n'{prp}'"
            else:
                reasoning.append(li)
        ...
        assert reasoning.strip(), f"Invalid llm empty reasoning: {reasoning} ({prp})"
    ...
    return reasoning, prompt, discarded
